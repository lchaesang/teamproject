...
app.get('/metaData', (req, res) => {
    ...
    console.log(from + ' ~ ' + to + ' getmetaData ');
...
// for showing srclist
app.get('/getsrclist', (req, res) => {

    // const query = `SELECT SRC, count(*) as count FROM ${DB_NAME} group by src having count(src)`;
    const query = `SELECT SRC FROM ${DB_NAME} group by src having count(src)`;

    connection.query(query, (err, rows) => {
        if (err) throw err;

        let srclist = [];
        let i, rowsLength = rows.length;
        for (i = 0; i < rowsLength; i++) {
            const row = rows[i];
            srclist.push(row.SRC);
        }
        res.send({ srclist });
    });
...
app.get('/getkeyword', (req, res) => {
    ...
    const srclist = req.query.srclist;
    const where2 = srclist.map((src) => { return `SRC= '${src}'` }).join(' OR ');
    const where = `(KEYWORD1 like '%${keyword}%' OR KEYWORD2 like '%${keyword}%' OR KEYWORD3 like '%${keyword}%' OR KEYWORD4 like '%${keyword}%' OR KEYWORD5 like '%${keyword}%')  and (date(DATE) between '${from}' and '${to}') and (${where2}) `;

    const query = `SELECT KEYWORD1, KEYWORD2, KEYWORD3, KEYWORD4, KEYWORD5  FROM ${DB_NAME} WHERE ${where} ORDER BY DATE DESC, SEQ DESC`;
...
app.get('/articlesBySearchAll', (req, res) => {
    ...
    console.log(from + ' ~ ' + to + ' keyword : ' + keyword);
    const where = `(KEYWORD1 like '%${keyword}%' OR KEYWORD2 like '%${keyword}%' OR KEYWORD3 like '%${keyword}%' OR KEYWORD4 like '%${keyword}%' OR KEYWORD5 like '%${keyword}%' OR CONTENTS like '%${keyword}%')  and (date(DATE) between '${from}' and '${to}') and (${where2})`;
